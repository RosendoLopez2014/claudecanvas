...
const CANVAS_CLAUDE_MD = [
  ...
  '',
  '## Critic Loop (MANDATORY when enabled)',
  '',
  'Call `critic_status` at the start of every task. If the critic is enabled, you MUST follow this workflow — no exceptions:',
  '',
  '### Before writing ANY code',
  '',
  '1. Formulate your implementation plan. You may save it to `docs/plans/` (plan files are exempt from the gate).',
  '2. **CALL `critic_review_plan`** with `planFile` (e.g. `docs/plans/my-plan.md`) or inline `plan` text. Do NOT skip this step.',
  '3. Wait for the critic verdict. Write/execute tools are BLOCKED until the critic approves (except writes to `docs/plans/`).',
  '',
  '### After implementing changes',
  '',
  '4. Call `critic_review_result` to submit your work for review. Git diff and diagnostics are collected automatically.',
  '',
  '### Other tools',
  '',
  '- `critic_status` — Check if the gate is active and what to do next.',
  '- `critic_override` — Bypass the gate with a logged reason (use sparingly).',
  '',
  '### Handling feedback',
  '',
  ...
  '- **If verdict is "revise"** — Address the issues listed. Fix critical and major issues first, then re-submit.',
  '- **If verdict is "reject"** — The approach has fundamental problems. Rethink your plan and re-submit.',
  '- **If verdict is "approve"** — The gate is released. Proceed with implementation.',
  '- **Do NOT argue with the critic.** Fix the issues or explain to the user why you disagree.',
  '',
  '**If you skip `critic_review_plan` and try to write code, your tool calls WILL fail.** Always check critic status first.',
].join('\n')
